...
class XModem extends FileTransfer {
				...
				this.transport
					...
					.then((ack) => {
						if (ack) {
							...
							reject(
								new Error("Failed to send packet after multiple attempts"),
							);
						}
					})
					.catch(reject);
		...
		return new Promise((resolve, reject) => {
			...
			const onDataReceived = async (data) => {
				try {
					...
					if (data[0] === SOH) {
						receivedBuffer = await this.handleSohPacket(
							data,
			...
			this.transport.on("data", onDataReceived);
			this.transport.send(Buffer.from([NAK])).catch((error) => {
				this.handleError(error);
				reject(error);
			});
		});
	...
	handleError(error) {
		...
		if (error.message.includes("Final ACK not received")) {
			this.logger.error(
				"Transfer failed: Final ACK not received. Aborting transfer.",
			);
		} else if (error.message.includes("Failed to send packet")) {
			this.logger.error(
				"Transfer failed: Unable to send packet after multiple attempts.",
			);
